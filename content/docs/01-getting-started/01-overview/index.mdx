...
You'll see the term **client** being used all over the place, when we say client we refer to an user of your application with a connection to Fungi's APIs.

A **connection** is an established [WebSocket](https://wikipedia.org/wiki/WebSocket) connection between a web browser (or any other client application) and Fungi's APIs.

For example, if user Alice has your app open on their laptop, their desktop and their phone, that would be 3 clients.
